...
from ..VectorDBEnums import (DistanceMethodEnums,PgVectorIndxTypeEnums,
                             PgVectorDistanceMethodEnums,PgVectorTableSchemaEnums)
import logging
...
class PGVectorProvider(vectordbInterface):
    ...
    async def is_collection_existed(self, collection_name :str) -> bool:
        record = None
        async with self.db_client() as session:
            async with session.begin():   # if you have error check this lien  
    ...
    async def create_vector_index(self,collection_name:str,
                                       index_type: str = PgVectorIndxTypeEnums.HNSW.value):
    ...
    async def insert_one(self, collection_name, text, vector, metadata = None, record_id = None):
        ...
        if not is_collection_existed:
            self.logger.error(f'Can not insert nwe record to non-existed collection: {collection_name}')
            return False
                ...
                async with session.begin():
                    insert_sql = sql_text(
                        'INSERT INTO  {collection_name} '
                            f'({PgVectorTableSchemaEnums.ID.value} {PgVectorTableSchemaEnums.TEXT.value} ,{PgVectorTableSchemaEnums.VECTOR.value},{PgVectorTableSchemaEnums.METADATA.value} ,{PgVectorTableSchemaEnums.CHUNK_ID.value})'
                            'VALUES (:text, :vector, :metadata, : chunk_id)'
                    )
                    meta_data_json = json.dumps(metadata,ensure_ascii=False) if metadata is not None else '{}'
                ...
                async with session.begin():
                    for i in range(0,len(texts),batch_size):
                        ...
                        for _text, _vector,_metadata,_record_id in zip(batch_text,batch_vectors,batch_metadata,batch_record_ids):
                            meta_data_json = json.dumps(_metadata,ensure_ascii=False) if _metadata is not None else '{}'
                            values.append({
                                "text" :_text,
