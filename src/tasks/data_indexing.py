...
async def _index_data_content(task_instance,project_id: int,do_reset:int):
    ...
    try:
        (db_engine,
            db_client,
            llm_provider_factory,vectordb_provider_factory,
            generation_client,embedding_client,
            vectordb_client,template_parser)= await get_setup_utils()
        
        logger.warning(f"Starting data indexing for project_id: {project_id} with do_reset: {do_reset}")
        ...
        while has_records:
            ...
            try:
                ...
                logger.error(f"Error indexing into vector DB for project_id: {project_id} at idx: {idx} with error: {str(e)}")
                raise
            ...
            if not is_inserted:
                ...
                raise Exception(f"Failed to insert into vector DB: {project_id}")
